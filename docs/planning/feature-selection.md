...
特徴量選定とは別に、IF 自体のハイパーパラメータも抽象化が必要（別 Issue）。

現状 `backend/analysis/anomaly.py` にハードコードされている:

| パラメータ | 現在の値 | 説明 |
|-----------|---------|------|
| `n_estimators` | 100 | 決定木の数 |
| `contamination` | "auto" | 想定される異常割合 |
| `max_samples` | デフォルト("auto") | 各木に使うサンプル数 |

これらを ModelDefinition 等に持たせ、将来的にユーザーが調整可能にする。

---

## 次のアクション

1. 実装 Issue を作成する（特徴量ビルダー実装）
2. 別 Issue を作成する（IF ハイパーパラメータ抽象化）
