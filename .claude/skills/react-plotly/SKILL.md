...
```
App.js                          # レイアウト + ビュー切替（dashboard / plot / settings）
...
└── DebugSettings.jsx           # デバッグ設定（CSVインポート + データ削除）— props なし
...
| 状態 | 管理場所 | 説明 |
|------|---------|------|
| `currentView` | App.js | `'dashboard'` / `'plot'` / `'settings'` ビュー切替 |
...
| CSVアップロード | `Upload.Dragger` | DebugSettings.jsx |
| 破壊操作の確認 | `Popconfirm` | DebugSettings.jsx |
| セクション分割 | `Card` | DebugSettings.jsx |
...
- **ヘッダー**: 固定上部。ナビゲーションMenu（ダッシュボード / プロット / 設定 切替）
...
- **ビュー切替**: 条件付きレンダリング（`currentView` に応じて `<Dashboard />` / `<PlotView />` / `<DebugSettings />` を表示）
